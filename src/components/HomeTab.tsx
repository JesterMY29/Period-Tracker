...
export const HomeTab: React.FC<HomeTabProps> = ({
  ...
  // Compute current cycle day if at least one period start exists
  ...
  return (
    <div className="space-y-8 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="card-refined p-6 sm:p-8 flex flex-col justify-between space-y-6">
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              <h3 className="font-serif text-3xl sm:text-4xl text-[#1a1a1a] font-medium tracking-tight">
                {currentCycleDayText}
              </h3>
              {periods.length > 0 && (
                <p className="font-mono text-xs text-[#1a1a1a]/60">
                  Last period start: {formatDisplayDate(periods[periods.length - 1].startDate)}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-[#1a1a1a]/10">
            <button
              ...
              className="bg-[#c47c7c] text-[#f8f7f4] w-full py-3 font-mono text-xs uppercase tracking-wider font-medium hover:opacity-90 transition-opacity flex items-center justify-center gap-2 cursor-pointer"
            >
        ...
        {/* Next Period Prediction Window Card */}
        <div className="card-refined p-6 sm:p-8 flex flex-col justify-between space-y-6">
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b border-[#1a1a1a]/10 pb-3">
            ...
            {prediction ? (
              <div className="space-y-3">
                <div className="p-4 bg-[#f8f7f4] border border-[#1a1a1a]/10 space-y-1">
                  ...
                  <p className="font-mono text-xs text-[#c47c7c]">
                    Estimated window (±{prediction.marginDays} days variation)
                  </p>
                </div>
                ...
                <p className="font-mono text-[11px] text-[#1a1a1a]/70 leading-relaxed p-3 bg-white border border-[#1a1a1a]/10">
                  ...
                  Predictions are calculated using your recorded period start dates. They are mathematical estimates and not medical advice.
                </p>
              </div>
            ) : (
              <div className="p-5 bg-[#f8f7f4] border border-dashed border-[#1a1a1a]/20 text-center space-y-2">
                ...
                <p className="font-mono text-xs font-semibold text-[#1a1a1a]">
                  No period predictions available yet
                </p>
                <p className="font-mono text-[11px] text-[#1a1a1a]/60">
                  Log your flow for at least one period date to begin calculating predicted period windows.
                </p>
              </div>
            )}
          </div>
        ...
        {todayLog ? (
          <div className="p-5 bg-[#f8f7f4] border border-[#1a1a1a]/10 font-mono text-xs space-y-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="px-3 py-1 bg-[#c47c7c] text-[#f8f7f4] font-medium uppercase tracking-wider">
                {todayLog.flow}
              </span>

              {todayLog.mood && (
                <>
                  <span className="font-semibold text-[#1a1a1a] ml-2">MOOD:</span>
                  <span className="px-3 py-1 bg-[#1a1a1a] text-[#f8f7f4] font-medium uppercase tracking-wider">
                    {todayLog.mood}
                  </span>
                </>
              )}
            ...
            {todayLog.symptoms && todayLog.symptoms.length > 0 && (
              <div className="space-y-1.5">
                <span className="font-semibold text-[#1a1a1a] block">SYMPTOMS:</span>
                <div className="flex flex-wrap gap-1.5">
                  {todayLog.symptoms.map((sym, idx) => (
                    <span
                      key={idx}
                      className="px-2.5 py-1 bg-white border border-[#1a1a1a]/15 text-[#1a1a1a]"
                    >
                      {sym}
                    </span>
                  ))}
            ...
            {todayLog.notes && (
              <div className="space-y-1 pt-2 border-t border-dashed border-[#1a1a1a]/15">
                <span className="font-semibold text-[#1a1a1a] block">NOTES:</span>
                <p className="p-3 bg-white border border-[#1a1a1a]/10 font-sans italic text-[#1a1a1a]/80 text-sm">
                  "{todayLog.notes}"
                </p>
              </div>
            )}
          ...
          <div className="p-6 bg-[#f8f7f4] border border-dashed border-[#1a1a1a]/20 text-center font-mono space-y-3">
            <p className="text-xs text-[#1a1a1a]/70">
              Track your daily flow, mood, and symptoms to build an accurate personal cycle record.
            </p>
            <button
              type="button"
              onClick={() => onOpenLogModal(todayStr)}
              className="text-xs font-semibold text-[#c47c7c] hover:underline cursor-pointer uppercase tracking-wider"
            >
              + Add entry for today
            </button>
          </div>
